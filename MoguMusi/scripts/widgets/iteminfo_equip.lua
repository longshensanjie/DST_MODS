...
local ItemInfoEquip = Class(Widget, function(self, equipslot, max_width, max_height)
	...
	self.prefab_image = self:AddChild(Image("images/inventoryimages1.xml", "hambat.tex"))
...
local atlas = GetInventoryItemAtlas(name)
	...
	if name and atlas then
		...
		self.prefab_image:SetTexture("images/inventoryimages1.xml", "hambat.tex")
	end
end
...
function ItemInfoEquip:OnUpdate(dt)
	-- bug真多
	if ThePlayer and ThePlayer.replica.inventory and ThePlayer.replica.inventory:GetEquippedItem(self.equipslot) ~= nil then
