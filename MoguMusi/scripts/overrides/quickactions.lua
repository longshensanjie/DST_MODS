...
-- 目标可以特殊攻击
local function IsAttackStaffable(target)
    return target:HasTag("MINE_workable")
        or (target.replica.combat and target.replica.combat:CanBeAttacked(ThePlayer))
        -- or target:HasTag("animal")
        -- or target:HasTag("pig")
        -- or target:HasTag("merm")
        -- or target:HasTag("monster")
        -- or target:HasTag("epic")
end

local function IsExtinguishable(target)
    return not IsAttackStaffable(target)                             -- 就你把我刚点燃的扑灭了呀
...
-- 是否特殊攻击武器
local function IsAttackStaffItem(item)
    local tagItem = ThePlayer.components.actioncontroller:GetItemFromCategory("STAFF")
    if tagItem then
        return item.prefab == tagItem.prefab
    end
    return false;
end

-- 获得特殊攻击武器
local function GetAttackStaffItem()
    for _, item in pairs(InventoryFunctions:GetPlayerInventory(true)) do
        if IsAttackStaffItem(item) then
            return item
        end
    end
    return nil
...
local QuickActions =
{
    ...
    QUICK_ACTION_ATTACK_STAFF =  QuickAction({itemfn = GetAttackStaffItem, modaction = "StaffAction"}),     -- 特殊攻击场景
}

QuickActions.QUICK_ACTION_ATTACK_STAFF.fn = IsAttackStaffable

QuickActions.QUICK_ACTION_ATTACK_STAFF.stringfn = function(item)
    return "攻击 (" .. item.name .. ")"
end
...
QuickActions.QUICK_ACTION_PIG_KING.stringfn = function(item)
    return "交易 " .. item.name .. " (" .. ItemFunctions:GetGoldValue(item) .. ")"
end
...
-- 直接去掉不要的操作
-- 没必要重置陷阱
QuickActions.QUICK_ACTION_TRAP = nil
-- 和富贵险中求冲突
QuickActions.QUICK_ACTION_BEEFALO = nil
QuickActions.QUICK_ACTION_KLAUS_SACK = nil
-- 修墙和更多动作冲突
QuickActions.QUICK_ACTION_WALLS = nil
-- 为什么要点燃蜗牛巢？
QuickActions.QUICK_ACTION_SLURTLEHOLE = nil
-- 敢烧战歌是吧
QuickActions.QUICK_ACTION_CAMPFIRE = nil
...
local function QuickActionFactory(self, target, quickAction)
    ...
    if not act and item and item.prefab == "staff_tornado" then
        act = BufferedAction(self.inst, target, ACTIONS.CASTSPELL, item)
    end
...
function ModActions.StaffAction(self, act)
...
local function CheckAbigailFlower()
    if not ThePlayer.prefab == "wendy" then return end
    local inv_items = InventoryFunctions:GetInventoryItems()
    for _, item in pairs(inv_items) do
        if item.prefab == "abigail_flower" then
            return item
        end
    end
    local backpack_items = InventoryFunctions:GetBackpackItems()
    for _, item in pairs(backpack_items) do
        if item.prefab == "abigail_flower" then
            return item
        end
    end
...
local function Init()
    ...
    --     local abigail_flower = CheckAbigailFlower()
