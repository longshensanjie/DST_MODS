...
local AUTO_RE_EQUIP_WEAPON = GetModConfigData("boas_AUTO_RE_EQUIP_WEAPON", MOD_EQUIPMENT_CONTROL.MODNAME)
local BUTTON_SHOW = GetModConfigData("boas_BUTTON_SHOW", MOD_EQUIPMENT_CONTROL.MODNAME)
local PREFERRED_FUEL = false
...
local function IsReturningToPlayer(item)
    ...
    if item.prefab == "nz_ring" then return true end
...
local function IsBoomerang(item)
    -- 支持乾坤圈
    return item.prefab == "boomerang" or item.prefab == "nz_ring"
end
...
local ItemTracker = Class(function(self, inst)
    ...
    if GetModConfigData("boas_AUTO_RE_EQUIP_ARMOR", MOD_EQUIPMENT_CONTROL.MODNAME) then
        AddTracker(IsArmor, "onremove", AutoReEquipArmor, true)
    end

    if GetModConfigData("boas_ELSE", MOD_EQUIPMENT_CONTROL.MODNAME) then
        AddTracker(IsBoomerang, "onremove", AutoCatchBoomerang, true)
    end

    if GetModConfigData("boas_AUTO_REFUEL_LIGHT_SOURCES", MOD_EQUIPMENT_CONTROL.MODNAME) then
