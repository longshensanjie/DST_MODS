local mapscale = GetModConfigData("minimap_size")
local position_str = GetModConfigData("minimap_position")
local margin_size_x = 235
...
MapWidget.OnZoomIn = function(self)
	...
	if minimap_small and self.shown then
		local zoomvalue = GLOBAL.TheWorld and GLOBAL.TheWorld.minimap and GLOBAL.TheWorld.minimap.MiniMap:GetZoom() or 1
		if type(zoomvalue) == "number" then
			minimap_small.mapscreenzoom = zoomvalue - 1
		end
	end
end
