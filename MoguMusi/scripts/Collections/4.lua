...
local function OnAfterLoad(controls)
  if _GameLoaded ~= true or _SimLoaded ~= true or _PlayerLoaded ~= true or _ControlsLoaded ~= true then
    ...
    local config = {
      lock_uncooked= false,--锁定未烹饪过的
      invert_controller= false,--反转控制器
      has_popup= true,--成分弹出
    }
...
local function ContainerPostConstruct(inst, prefab)
  ...
  if not tableContains(inst.inst.prefab, {'cookpot', 'portablecookpot', 'deluxpot', 'medal_cookpot', 'archive_cookpot'}) then
    return false
  end
