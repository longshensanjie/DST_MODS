...
local SHOWSTATNUMBERS = true--显示健康，饥饿和理智的数字
local SHOWDETAILEDSTATNUMBERS = SHOWSTATNUMBERS == "Detailed"
local SHOWMAXONNUMBERS = true--显示上限数值，便于直观的了解数据
local SHOWCLOCKTEXT = true--在时钟（日期编号）和季节时钟（当前季节）上显示文字
local SHOWTEMPERATURE =true--显示玩家温度
local SHOWNAUGHTINESS = true and not DST--淘气值
local SHOWWORLDTEMP = GetModConfigData("shizhong_SHOWWORLDTEMP")--显示世界温度（火灾之类除外）
local SHOWTEMPBADGES = true--显示图标提示哪个是温度
local SHOWBEAVERNESS = true --显示伐木工伍迪的木头值，其他角色不会显示.
local HIDECAVECLOCK =true--在洞穴显示时钟，只在巨人国有效
local SHOWMOON = GetModConfigData("shizhong_SHOWMOON")--在白天和傍晚显示月圆图标
local SHOWMOONDAY = SHOWMOON > 1
local SHOWMOONDUSK = SHOWMOON > 0
local SHOWWANINGMOON = true--提示月圆和月满，用图标区分
local SHOWNEXTFULLMOON = true--在鼠标滑过图标上显示下一个满月时间
local FLIPMOON = false--翻转月亮相位（false像在北半球一样展示月亮，true像在南半球一样展示月亮）
local UNIT = GetModConfigData("sw_shizhong_UNIT") or "T"  --默认设置为游戏单位
local SEASONOPTIONS ="Clock"--添加一个图标显示季节
...
local HUDSCALEFACTOR = GetModConfigData("shizhong_HUDSCALEFACTOR")*.01--你可以调整HUD的界面，以及时钟显示的大小
