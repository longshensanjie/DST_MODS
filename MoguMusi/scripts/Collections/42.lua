...
local amulet_prefab = "yellowamulet"
local modconfig_autoaddfuelpercent

local judge = GetModConfigData("sw_autoadd")
if judge == true then
                ...
                if item and _G.ThePlayer.replica.inventory:IsHolding(item) and
					...
					if _G.next(fuel) then
						if percentused <= 2 then -- 2%强制取下
							...
							Unequip(item)
							break
						elseif percentused <= 8 then -- 小于8%强制加燃料
...
local function fn(inst)
    ...
    if item and item.prefab == amulet_prefab then
