local language              = "chinese_s"
local speedy_mode           = GetModConfigData("autocook_sel")
local cookpots_num_divisor  = GetModConfigData("autocook_selnum")

local key_2                 = GetModConfigData("autocook_Ctrl") or -1

local btn_last = GetModConfigData("autocook_last")

local laggy_mode_on = not GetModConfigData("autocook_sel")
...
local function Say(str)
    local talker = GLOBAL.ThePlayer and GLOBAL.ThePlayer.components.talker
    if talker then
        talker:Say(str)
    end
end
...
local function GetOpenContainers()
    if GLOBAL.ThePlayer.components.inventory ~= nil then
        return GLOBAL.ThePlayer.components.inventory.opencontainers

    elseif GLOBAL.ThePlayer.HUD ~= nil then
        local containers = {}
        for k, v in pairs(GLOBAL.ThePlayer.HUD.controls.containers) do
            if v ~= nil and v.inst.entity:IsVisible() and v.isopen and k:IsValid() then -- Check isopen
                containers[k] = true
            end
        end
        local overflow = GLOBAL.ThePlayer.replica.inventory:GetOverflowContainer()
...
local function Wait(act)
    ...
    until not (GLOBAL.ThePlayer.sg and GLOBAL.ThePlayer.sg:HasStateTag("moving")) and not GLOBAL.ThePlayer:HasTag("moving")
        and (not (act and act.action == GLOBAL.ACTIONS.RUMMAGE) or GLOBAL.ThePlayer:HasTag("idle") and not GLOBAL.ThePlayer.components.playercontroller:IsDoingOrWorking())
    if laggy_mode_on then GLOBAL.Sleep(0.3) end
end
...
local function SendLeftClickAction(act, target)
    ...
    local pos = act:GetActionPoint() or GLOBAL.ThePlayer:GetPosition()
    local function send()
        GLOBAL.SendRPCToServer(GLOBAL.RPC.LeftClick, act.action.code, pos.x, pos.z, target, true)
    end
...
local function find_cookware(prefab, canttags)
    return GLOBAL.FindEntity(GLOBAL.ThePlayer, STEWER_RANGE, function(inst)
...
local function GetItems(required_items)
    required_items = GLOBAL.shallowcopy(required_items)
    local final_steps = {}
    for _, container in orderedPairs(GetDefaultCheckingContainers()) do
        if type(container) == "table" and container.GetItems then
            local items = container:GetItems()
            for slot, item in orderedPairs(items) do
                if required_items[item.prefab] then
                    ...
                    if GLOBAL.IsTableEmpty(required_items) then
                        items.__orderedIndex = nil
                        return final_steps
                    end
                end
    ...
    while not (cookware:IsValid() and container:IsEmpty()) do
        for i = 1, container:GetNumSlots() do
            if container:GetItemInSlot(i) then
                if GLOBAL.ThePlayer.replica.inventory:IsFull() then
                    local backpack = GLOBAL.ThePlayer.replica.inventory:GetOverflowContainer()
                    if backpack and not backpack:IsFull() then
                        GLOBAL.SendRPCToServer(GLOBAL.RPC.MoveItemFromAllOfSlot, i, cookware, backpack.inst)
                    else
                        Say(GetString("cant_move_out_items"))
                        return false
                    end
                else
                    GLOBAL.SendRPCToServer(GLOBAL.RPC.MoveItemFromAllOfSlot, i, cookware)
                end
            end
        end
        GLOBAL.Sleep(SLEEP_TIME)
...
local function GetHarvestTarget()
    for i = #harvestinglist, 1, -1 do
        ...
        if not (
            ...
            and GLOBAL.ThePlayer:IsNear(cookware, MAX_HARVEST_DIST)
        ) then
...
local function DoRummage(target)
    local act = GLOBAL.BufferedAction(GLOBAL.ThePlayer, target, GLOBAL.ACTIONS.RUMMAGE)
    while not (target.replica.container and target.replica.container:IsOpenedBy(GLOBAL.ThePlayer)) do
        SendActionAndWait(act, target)
    end
    table.removearrayvalue(harvestinglist, target)
end
...
local function TryWalkTo(target)
    if target and not target:IsNear(GLOBAL.ThePlayer, 2) then
        local pos = target:GetPosition()
        local act = GLOBAL.BufferedAction(GLOBAL.ThePlayer, target, GLOBAL.ACTIONS.WALKTO, nil, pos)
        if not GLOBAL.ThePlayer:HasTag("moving") and GLOBAL.ThePlayer:HasTag("idle") then
            SendLeftClickAction(act)
            GLOBAL.ThePlayer:DoTaskInTime(0, function() SendLeftClickAction(act, target) end) -- From Advanced Controls
        end
    end
end
...
local function HarvestOnly(endless)
    ...
    local function harvest_thread()
        while GLOBAL.ThePlayer:IsValid() do
            if endless then
                local target = GLOBAL.FindEntity(GLOBAL.ThePlayer, STEWER_RANGE, find_endless_target, COOKWARE_MUSTTAGS, COOKWARE_CANTTAGS)
                if target then
                    DoHarvest(target)
                else
                    GLOBAL.Sleep(SLEEP_TIME)
    ...
    if ac_thread then
        ...
        ac_thread = GLOBAL.ThePlayer:StartThread(harvest_thread)
    end
end
...
local function DoButtonFn(container, single)
    local container_replica = container.replica.container
    if single then
        container_replica.widget.buttoninfo.fn(container, GLOBAL.ThePlayer)
        return
    end

    while container:IsValid() and container_replica:IsOpenedBy(GLOBAL.ThePlayer) do
        container_replica.widget.buttoninfo.fn(container, GLOBAL.ThePlayer)
        GLOBAL.Sleep(SLEEP_TIME)
    end
...
local function DoFillUpAndCook(cookware, items, cookwares)
        ...
        if speedy_mode then
            ...
            if steps then
                for _, data in ipairs(steps) do
                    if data.container == GLOBAL.ThePlayer then
                        GLOBAL.SendRPCToServer(GLOBAL.RPC.MoveInvItemFromAllOfSlot, data.slot, cookware)
                    else
                        GLOBAL.SendRPCToServer(GLOBAL.RPC.MoveItemFromAllOfSlot, data.slot, data.container, cookware)
                    end
                end
    ...
    until not cookware:IsValid()
        or not container_replica:IsOpenedBy(GLOBAL.ThePlayer)
...
local function AutoCooking(items, cookwares)
    ...
    ac_thread = GLOBAL.ThePlayer:StartThread(function()
...
local function GetStartingCookwares(first_cookware, items, cooking_type)
    ...
    local ents = GLOBAL.TheSim:FindEntities(x, 0, z, STEWER_RANGE, COOKWARE_MUSTTAGS, ismasterchef and COOKWARE_CANTTAGS or NO_MASTERCHEF_CANTTAGS)

    if not should_harvest or cooking_type ~= COOKING or GLOBAL.TheInput:IsKeyDown(GLOBAL.KEY_SHIFT) then
        for _, ent in ipairs(ents) do
    ...
    cookingtime = cookingtime * GLOBAL.TUNING.BASE_COOK_TIME

    if target_prefab == "portablecookpot" then
        cookingtime = cookingtime * GLOBAL.TUNING.PORTABLE_COOK_POT_TIME_MULTIPLIER

    elseif target_prefab == "medal_cookpot" and rawget(GLOBAL, TUNING_MEDAL) and TUNING_MEDAL.PORTABLE_COOK_POT_TIME_MULTIPLIER then  -- For Functional Medal Mod
        cookingtime = cookingtime * GLOBAL.TUNING_MEDAL.PORTABLE_COOK_POT_TIME_MULTIPLIER
...
local function Start(use_last_recipe, override_data)
    ...
    if override_data then
        items, cooking_type, start_cookware = GLOBAL.unpack(override_data)
    else
    ...
    ismasterchef = GLOBAL.ThePlayer:HasTag("masterchef")
...
AddComponentPostInit("playercontroller", function(self)
    ...
    self.OnControl = function(self, control, down, ...)
        ...
        if down and InGame() then
            if ac_thread then
                if interrupt_control or mouse_control and not GLOBAL.TheInput:GetHUDEntityUnderMouse() then
                    StopCooking()
                end
            elseif key_2 ~= -1 and control == GLOBAL.CONTROL_PRIMARY and GLOBAL.TheInput:IsKeyDown(key_2) then
...
if btn_last then
    GLOBAL.TheInput:AddKeyDownHandler(btn_last, function()
        if not InGame() then
            return
        end
