...
local CTRL = false
local KEYBOARDTOGGLEKEY = GetModConfigData("KEYBOARDTOGGLEKEY") or -1
local GEOMETRYTOGGLEKEY = GetModConfigData("GEOMETRYTOGGLEKEY") or -1
local SNAPGRIDKEY = GetModConfigData("SNAPGRIDKEY") or -1
...
local CONTROLLEROFFSET = false
local SMARTSPACING = false
...
for action,_ in pairs(RMB_ACTION_GRID_SPACING) do
	SetGridForRmbAction(action, not GetModConfigData("sw_snapping") and GetConfig("ACTION_"..action, true, "boolean"))
end
	...
	if DST then
		AddClassPostConstruct("screens/playerstatusscreen", function(PlayerStatusScreen)
			...
			function PlayerStatusScreen:GetHelpText()
				local control_string = SHOWMENU and "打开几何设置" or "切换几何形状"
