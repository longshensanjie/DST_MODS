...
function ItemFunctions:IsLightSource(item)
    return not item:HasTag("fueldepleted")
       and (item:HasTag(FUELTYPE.CAVE .. "_fueled")
            ...
            or item:HasTag("fire")
            or item:HasTag("lighter"))  -- 改为标签了
            -- -- 支持神话灯笼
            -- or (item and item.prefab == "myth_redlantern")
            -- -- 支持成就和世界活动
            -- -- 如何获取红灯笼的耐久？0%不要自动切换
            -- or (item and item.prefab == "redlantern")
end
...
function ItemFunctions:IsCane(item)
    return(self:GetWalkspeedMult(item) > 1 or (item and item:HasTag("dumbbell")))
    and self:GetEquipSlot(item) == EQUIPSLOTS.HANDS         -- 并且装备在手
...
local ModWeapons = {"windyknife","magic_wand_cj","magic_wand","tz_yezhao","xe_writingbrush"}
local function checkModWeapons(item)
    if item then
        for _, weapon in pairs(ModWeapons) do
            if weapon == item.prefab then
                return true
            end
        end
    end
...
function ItemFunctions:IsMeleeWeapon(item)
    ...
    --    and not self:IsStaff(item)
       or self:IsStaff(item)
       or checkModWeapons(item)
end
