local function a(b, c)
    local d = 'local player = %s if player == nil then UserToPlayer("' .. _G.TOOMANYITEMS.DATA.ThePlayerUserId ..
    ...
    local f = "')) beef.Transform:SetPosition(x,y,z)"
    return d .. b .. e .. c .. f
end
function gotoswitch(g, h)
    g = g or ""
    h = h or ""
    return
        'local player = %s if player ~= nil then local function tmi_goto(prefab) if player.Physics ~= nil then player.Physics:Teleport(prefab.Transform:GetWorldPosition()) else player.Transform:SetPosition(prefab.Transform:GetWorldPosition()) end end local target = c_findnext("' ..
            g .. '") if target == nil then target = c_findnext("' .. h .. '") end tmi_goto(target) end'
end
local i = TheWorld.meta.session_identifier or 'world'
local j = TOOMANYITEMS.TELEPORT_DATA_FILE .. 'toomanyitemsplus_teleport_save_' .. i;
local function k(l)
    if l and type(l) == 'number' then
        local m = TOOMANYITEMS.TELEPORT_DATA[l] and TOOMANYITEMS.TELEPORT_DATA[l]['x']
        local n = TOOMANYITEMS.TELEPORT_DATA[l] and TOOMANYITEMS.TELEPORT_DATA[l]['z']
        if m and n and type(m) == 'number' and type(n) == 'number' then
            SendCommand(string.format('local player = %s if player == nil then UserToPlayer("' ..
                                          _G.TOOMANYITEMS.DATA.ThePlayerUserId .. '").components.talker:Say("' ..
                                          STRINGS.TOO_MANY_ITEMS_UI.PLAYER_NOT_ON_SLAVE_TIP ..
                                          '") end if player ~= nil then if player.Physics ~= nil then player.Physics:Teleport(' ..
                                          m .. ', ' .. '0, ' .. n .. ') else player.Transform:SetPosition(' .. m .. ', ' ..
                                          '0, ' .. n .. ') end end', GetCharacter()))
        end
    end
end
local function o(l)
    local m, p, n = ThePlayer.Transform:GetWorldPosition()
    if m and n and l and type(m) == 'number' and type(n) == 'number' and type(l) == 'number' then
        TOOMANYITEMS.TELEPORT_DATA[l] = {}
        TOOMANYITEMS.TELEPORT_DATA[l]['x'] = m;
        TOOMANYITEMS.TELEPORT_DATA[l]['z'] = n;
        if TOOMANYITEMS.G_TMIP_DATA_SAVE == 1 then
            TOOMANYITEMS.SaveData(j, TOOMANYITEMS.TELEPORT_DATA)
        end
    end
end
local function q(l)
...
local function r(s)
    ...
    for u = 1, 10 do
        t[u] = {
            ...
            pos = 'all',
            name = '[ ' .. u .. ' ]',
            tip = STRINGS.TOO_MANY_ITEMS_UI.DEBUG_TELEPORT_SLOT .. ' ' .. u,
            fn = {
                TeleportNum = u,
                TeleportFn = s
            }
...
local function v()
    ...
    x = x + 1;
    for p, z in pairs(y) do
        w[x] = {
            beta = false,
            pos = 'all',
            name = z .. (z == 1 and STRINGS.UI.PLAYERSTATUSSCREEN.AGE_DAY or STRINGS.UI.PLAYERSTATUSSCREEN.AGE_DAYS),
            tip = STRINGS.TOO_MANY_ITEMS_UI.DEBUG_TIME_SKIPDAYTIP .. ' ' .. z .. ' ' ..
                (z == 1 and STRINGS.UI.PLAYERSTATUSSCREEN.AGE_DAY or STRINGS.UI.PLAYERSTATUSSCREEN.AGE_DAYS),
            fn = {'confirm', 'c_skip(' .. z .. ')', STRINGS.TOO_MANY_ITEMS_UI.DEBUG_TIME_SKIPDAYSCONFIRM,
                  string.format(STRINGS.TOO_MANY_ITEMS_UI.DEBUG_TIME_SKIPDAYSCONFIRMTIP, z)}
        }
        x = x + 1
    end
    local A = {0.5, 1, 2, 3, 4}
    for p, B in pairs(A) do
        w[x] = {
            beta = false,
            pos = 'all',
            name = B .. STRINGS.TOO_MANY_ITEMS_UI.DEBUG_TIME_0PRUM,
            tip = string.format(STRINGS.TOO_MANY_ITEMS_UI.DEBUG_TIME_0PRUMTIP, B * 100) .. '%',
            fn = {'confirm', 'TheSim:SetTimeScale(' .. B .. ') print("Speed is now ", TheSim:GetTimeScale())',
                  STRINGS.TOO_MANY_ITEMS_UI.DEBUG_TIME_SPEEDRUNCONFIRM,
                  string.format(STRINGS.TOO_MANY_ITEMS_UI.DEBUG_TIME_SPEEDRUNCONFIRMTIP, B * 100) .. '% ?'}
        }
        x = x + 1
    end
    return w
...
return {{
    ...
    list = {{
        ...
        fn = 'local player = %s if player == nil then UserToPlayer("' .. _G.TOOMANYITEMS.DATA.ThePlayerUserId ..
            '").components.talker:Say("' .. STRINGS.TOO_MANY_ITEMS_UI.PLAYER_NOT_ON_SLAVE_TIP ..
            '") end if player ~= nil and player.components.locomotor then player.components.locomotor:SetExternalSpeedMultiplier(player, "c_speedmult", 0.6) end'
    }, {
        beta = false,
        pos = 'all',
        ...
        fn = 'local player = %s if player == nil then UserToPlayer("' .. _G.TOOMANYITEMS.DATA.ThePlayerUserId ..
            '").components.talker:Say("' .. STRINGS.TOO_MANY_ITEMS_UI.PLAYER_NOT_ON_SLAVE_TIP ..
            '") end if player ~= nil and player.components.locomotor then player.components.locomotor:SetExternalSpeedMultiplier(player, "c_speedmult", 1) end'
    }, {
        beta = false,
        pos = 'all',
        ...
        fn = 'local player = %s if player == nil then UserToPlayer("' .. _G.TOOMANYITEMS.DATA.ThePlayerUserId ..
            '").components.talker:Say("' .. STRINGS.TOO_MANY_ITEMS_UI.PLAYER_NOT_ON_SLAVE_TIP ..
            '") end if player ~= nil and player.components.locomotor then player.components.locomotor:SetExternalSpeedMultiplier(player, "c_speedmult", 2) end'
    }, {
        beta = false,
        pos = 'all',
        ...
        fn = 'local player = %s if player == nil then UserToPlayer("' .. _G.TOOMANYITEMS.DATA.ThePlayerUserId ..
            '").components.talker:Say("' .. STRINGS.TOO_MANY_ITEMS_UI.PLAYER_NOT_ON_SLAVE_TIP ..
            '") end if player ~= nil and player.components.locomotor then player.components.locomotor:SetExternalSpeedMultiplier(player, "c_speedmult", 3) end'
    }, {
        beta = false,
        pos = 'all',
        ...
        fn = 'local player = %s if player == nil then UserToPlayer("' .. _G.TOOMANYITEMS.DATA.ThePlayerUserId ..
            '").components.talker:Say("' .. STRINGS.TOO_MANY_ITEMS_UI.PLAYER_NOT_ON_SLAVE_TIP ..
            '") end if player ~= nil and player.components.locomotor then player.components.locomotor:SetExternalSpeedMultiplier(player, "c_speedmult", 4) end'
    }}
}, {
    ...
    list = {{
        ...
        fn = {'confirm',
              'local player = %s if player == nil then UserToPlayer("' .. _G.TOOMANYITEMS.DATA.ThePlayerUserId ..
            '").components.talker:Say("' .. STRINGS.TOO_MANY_ITEMS_UI.PLAYER_NOT_ON_SLAVE_TIP ..
            '") end player:PushEvent("death") player.deathpkname = "' .. STRINGS.TOO_MANY_ITEMS_UI.TMIP_CONSOLE .. '"',
              STRINGS.TOO_MANY_ITEMS_UI.DEBUG_CHARACTER_KILLTIP,
              STRINGS.TOO_MANY_ITEMS_UI.DEBUG_CHARACTER_KILLCONFIRMTIP}
    }, {
        beta = false,
        pos = 'all',
        ...
        fn = {'confirm',
              'local player = %s if player == nil then UserToPlayer("' .. _G.TOOMANYITEMS.DATA.ThePlayerUserId ..
            '").components.talker:Say("' .. STRINGS.TOO_MANY_ITEMS_UI.PLAYER_NOT_ON_SLAVE_TIP ..
            '") end player:PushEvent("respawnfromghost") player.rezsource = "' .. STRINGS.TOO_MANY_ITEMS_UI.TMIP_CONSOLE ..
            '"', STRINGS.TOO_MANY_ITEMS_UI.DEBUG_CHARACTER_REBIRTHTIP,
              STRINGS.TOO_MANY_ITEMS_UI.DEBUG_CHARACTER_REBIRTHCONFIRMTIP}
    }, {
        beta = false,
        pos = 'all',
        name = STRINGS.TOO_MANY_ITEMS_UI.DEBUG_CHARACTER_DESPAWN,
        tip = STRINGS.TOO_MANY_ITEMS_UI.DEBUG_CHARACTER_DESPAWNTIP,
        fn = {'confirm', 'c_despawn(%s)', STRINGS.TOO_MANY_ITEMS_UI.DEBUG_CHARACTER_DESPAWN,
        ...
        fn = {'confirm',
              ...
              STRINGS.TOO_MANY_ITEMS_UI.DEBUG_CHARACTER_GATHERTIP,
              STRINGS.TOO_MANY_ITEMS_UI.DEBUG_CHARACTER_GATHERCONFIRMTIP}
    }, {
        beta = false,
        pos = 'all',
        ...
        fn = 'local player = %s if player == nil then UserToPlayer("' .. _G.TOOMANYITEMS.DATA.ThePlayerUserId ..
            '").components.talker:Say("' .. STRINGS.TOO_MANY_ITEMS_UI.PLAYER_NOT_ON_SLAVE_TIP ..
            '") end if player ~= nil and player.components.health then player.components.health:SetPenalty(player.components.health.penalty + 0.25) player.components.health:ForceUpdateHUD(true) end'
    }, {
        beta = false,
        pos = 'all',
    ...
    list = {{
        ...
        beta = false,
        pos = 'all',
        name = STRINGS.TOO_MANY_ITEMS_UI.HUXI_KILL,
        tip = STRINGS.TOO_MANY_ITEMS_UI.HUXI_KILL_TIP,
        fn = 'local player = %s if player == nil then UserToPlayer("' .. _G.TOOMANYITEMS.DATA.ThePlayerUserId ..
            '").components.talker:Say("' .. STRINGS.TOO_MANY_ITEMS_UI.PLAYER_NOT_ON_SLAVE_TIP ..
            '") end if player and player.Transform then local x, y, z = player.Transform:GetWorldPosition() local ents = TheSim:FindEntities(x, y, z, 20 , nil, {"FX","DECOR","INLIMBO","NOCLICK"}) for _, v in pairs(ents) do if not v:HasTag("player") and not v:HasTag("wall") and not v:HasTag("boat") then if v.components and v.components.health then v.components.health:Kill() end end end end'
        }, {
        beta = false,
        pos = 'all',
        name = STRINGS.TOO_MANY_ITEMS_UI.DEBUG_ENTITY_EXTINGUISH,
        tip = STRINGS.TOO_MANY_ITEMS_UI.DEBUG_ENTITY_EXTINGUISHTIP,
        fn = 'local a=%s;if a==nil then UserToPlayer("' .. _G.TOOMANYITEMS.DATA.ThePlayerUserId ..
            '").components.talker:Say("' .. STRINGS.TOO_MANY_ITEMS_UI.PLAYER_NOT_ON_SLAVE_TIP ..
    ...
    list = {{
        ...
        fn = {'confirm', 'c_reset()', STRINGS.TOO_MANY_ITEMS_UI.DEBUG_SERVER_RESET,
        ...
        fn = {'confirm', 'c_regenerateworld()', STRINGS.TOO_MANY_ITEMS_UI.DEBUG_SERVER_REGENERATE,
        ...
        fn = {'confirm', 'if TheNet:GetIsServer() and TheNet:GetServerIsDedicated() then c_shutdown(true) end',
              STRINGS.TOO_MANY_ITEMS_UI.DEBUG_SERVER_SHUTDOWN, STRINGS.TOO_MANY_ITEMS_UI.DEBUG_SERVER_SHUTDOWNTIP}
    }, {
        beta = false,
        pos = 'all',
    ...
    list = {{
        ...
        fn = {'confirm',
              'ApplySpecialEvent("none") TheWorld.topology.overrides.specialevent = "none" c_save() c_announce("' ..
            STRINGS.TOO_MANY_ITEMS_UI.DEBUG_SERVER_RESETTIP .. STRINGS.TOO_MANY_ITEMS_UI.DEBUG_EVENT_TIPTEXT ..
            ...
            '") TheWorld:DoTaskInTime(5, function() if TheWorld ~= nil and TheWorld.ismastersim then TheNet:SendWorldRollbackRequestToServer(0) end end)',
              STRINGS.TOO_MANY_ITEMS_UI.DEBUG_EVENT_TIPTEXT .. STRINGS.UI.SANDBOXMENU.NONE,
        ...
        fn = {'confirm',
              'ApplySpecialEvent("default") TheWorld.topology.overrides.specialevent = "default" c_save() c_announce("' ..
            STRINGS.TOO_MANY_ITEMS_UI.DEBUG_SERVER_RESETTIP .. STRINGS.TOO_MANY_ITEMS_UI.DEBUG_EVENT_TIPTEXT ..
            STRINGS.UI.SANDBOXMENU.SPECIAL_EVENTS.DEFAULT ..
            '") TheWorld:DoTaskInTime(5, function() if TheWorld ~= nil and TheWorld.ismastersim then TheNet:SendWorldRollbackRequestToServer(0) end end)',
              STRINGS.TOO_MANY_ITEMS_UI.DEBUG_EVENT_TIPTEXT .. STRINGS.UI.SANDBOXMENU.SPECIAL_EVENTS.DEFAULT,
              STRINGS.TOO_MANY_ITEMS_UI.DEBUG_EVENT_TEXTCHANGESTIP}
    }, {
        beta = false,
        pos = 'all',
        ...
        fn = {'confirm',
              'ApplySpecialEvent("hallowed_nights") TheWorld.topology.overrides.specialevent = "hallowed_nights" c_save() c_announce("' ..
            STRINGS.TOO_MANY_ITEMS_UI.DEBUG_SERVER_RESETTIP .. STRINGS.TOO_MANY_ITEMS_UI.DEBUG_EVENT_TIPTEXT ..
            STRINGS.UI.SANDBOXMENU.SPECIAL_EVENTS.HALLOWED_NIGHTS ..
            '") TheWorld:DoTaskInTime(5, function() if TheWorld ~= nil and TheWorld.ismastersim then TheNet:SendWorldRollbackRequestToServer(0) end end)',
              STRINGS.TOO_MANY_ITEMS_UI.DEBUG_EVENT_TIPTEXT .. STRINGS.UI.SANDBOXMENU.SPECIAL_EVENTS.HALLOWED_NIGHTS,
              STRINGS.TOO_MANY_ITEMS_UI.DEBUG_EVENT_TEXTCHANGESTIP}
    }, {
        beta = false,
        pos = 'all',
        ...
        fn = {'confirm',
              'ApplySpecialEvent("winters_feast") TheWorld.topology.overrides.specialevent = "winters_feast" c_save() c_announce("' ..
            STRINGS.TOO_MANY_ITEMS_UI.DEBUG_SERVER_RESETTIP .. STRINGS.TOO_MANY_ITEMS_UI.DEBUG_EVENT_TIPTEXT ..
            STRINGS.UI.SANDBOXMENU.SPECIAL_EVENTS.WINTERS_FEAST ..
            '") TheWorld:DoTaskInTime(5, function() if TheWorld ~= nil and TheWorld.ismastersim then TheNet:SendWorldRollbackRequestToServer(0) end end)',
              STRINGS.TOO_MANY_ITEMS_UI.DEBUG_EVENT_TIPTEXT .. STRINGS.UI.SANDBOXMENU.SPECIAL_EVENTS.WINTERS_FEAST,
              STRINGS.TOO_MANY_ITEMS_UI.DEBUG_EVENT_TEXTCHANGESTIP}
    }, {
        beta = false,
        pos = 'all',
        ...
        fn = {'confirm',
              'ApplySpecialEvent("year_of_the_gobbler") TheWorld.topology.overrides.specialevent = "year_of_the_gobbler" c_save() c_announce("' ..
            STRINGS.TOO_MANY_ITEMS_UI.DEBUG_SERVER_RESETTIP .. STRINGS.TOO_MANY_ITEMS_UI.DEBUG_EVENT_TIPTEXT ..
            STRINGS.UI.SANDBOXMENU.SPECIAL_EVENTS.YOTG ..
            '") TheWorld:DoTaskInTime(5, function() if TheWorld ~= nil and TheWorld.ismastersim then TheNet:SendWorldRollbackRequestToServer(0) end end)',
              STRINGS.TOO_MANY_ITEMS_UI.DEBUG_EVENT_TIPTEXT .. STRINGS.UI.SANDBOXMENU.SPECIAL_EVENTS.YOTG,
              STRINGS.TOO_MANY_ITEMS_UI.DEBUG_EVENT_TEXTCHANGESTIP}
    }, {
        beta = false,
        pos = 'all',
        ...
        fn = {'confirm',
              'ApplySpecialEvent("year_of_the_varg") TheWorld.topology.overrides.specialevent = "year_of_the_varg" c_save() c_announce("' ..
            STRINGS.TOO_MANY_ITEMS_UI.DEBUG_SERVER_RESETTIP .. STRINGS.TOO_MANY_ITEMS_UI.DEBUG_EVENT_TIPTEXT ..
            STRINGS.UI.SANDBOXMENU.SPECIAL_EVENTS.YOTV ..
            '") TheWorld:DoTaskInTime(5, function() if TheWorld ~= nil and TheWorld.ismastersim then TheNet:SendWorldRollbackRequestToServer(0) end end)',
              STRINGS.TOO_MANY_ITEMS_UI.DEBUG_EVENT_TIPTEXT .. STRINGS.UI.SANDBOXMENU.SPECIAL_EVENTS.YOTV,
              STRINGS.TOO_MANY_ITEMS_UI.DEBUG_EVENT_TEXTCHANGESTIP}
    }, {
        beta = false,
        pos = 'all',
        ...
        fn = {'confirm',
              'ApplySpecialEvent("year_of_the_pig") TheWorld.topology.overrides.specialevent = "year_of_the_pig" c_save() c_announce("' ..
            STRINGS.TOO_MANY_ITEMS_UI.DEBUG_SERVER_RESETTIP .. STRINGS.TOO_MANY_ITEMS_UI.DEBUG_EVENT_TIPTEXT ..
            STRINGS.UI.SANDBOXMENU.SPECIAL_EVENTS.YOTP ..
            '") TheWorld:DoTaskInTime(5, function() if TheWorld ~= nil and TheWorld.ismastersim then TheNet:SendWorldRollbackRequestToServer(0) end end)',
              STRINGS.TOO_MANY_ITEMS_UI.DEBUG_EVENT_TIPTEXT .. STRINGS.UI.SANDBOXMENU.SPECIAL_EVENTS.YOTP,
              STRINGS.TOO_MANY_ITEMS_UI.DEBUG_EVENT_TEXTCHANGESTIP}
    }, {
        beta = false,
        pos = 'all',
        ...
        fn = {'confirm',
            ...
            STRINGS.TOO_MANY_ITEMS_UI.DEBUG_SERVER_RESETTIP .. STRINGS.TOO_MANY_ITEMS_UI.DEBUG_EVENT_TIPTEXT ..
            STRINGS.UI.SANDBOXMENU.SPECIAL_EVENTS.YOTC ..
            '") TheWorld:DoTaskInTime(5, function() if TheWorld ~= nil and TheWorld.ismastersim then TheNet:SendWorldRollbackRequestToServer(0) end end)',
    ...
    list = {{
        ...
        fn = {'confirm', 'c_rollback(1)', STRINGS.UI.BUILTINCOMMANDS.ROLLBACK.PRETTYNAME,
              STRINGS.UI.BUILTINCOMMANDS.ROLLBACK.DESC}
    }, {
        beta = false,
        pos = 'all',
        name = STRINGS.TOO_MANY_ITEMS_UI.DEBUG_SERVER_ROLLBACK_TWO,
        tip = STRINGS.TOO_MANY_ITEMS_UI.DEBUG_SERVER_ROLLBACK_TWOTIP,
        fn = {'confirm', 'c_rollback(2)', STRINGS.UI.BUILTINCOMMANDS.ROLLBACK.PRETTYNAME,
              STRINGS.UI.BUILTINCOMMANDS.ROLLBACK.DESC}
    }, {
        beta = false,
        pos = 'all',
        ...
        fn = {'confirm', 'c_rollback(3)', STRINGS.UI.BUILTINCOMMANDS.ROLLBACK.PRETTYNAME,
              STRINGS.UI.BUILTINCOMMANDS.ROLLBACK.DESC}
    }, {
        beta = false,
        pos = 'all',
        ...
        fn = {'confirm', 'c_rollback(4)', STRINGS.UI.BUILTINCOMMANDS.ROLLBACK.PRETTYNAME,
              STRINGS.UI.BUILTINCOMMANDS.ROLLBACK.DESC}
    }, {
        beta = false,
        pos = 'all',
        ...
        fn = {'confirm', 'c_rollback(5)', STRINGS.UI.BUILTINCOMMANDS.ROLLBACK.PRETTYNAME,
              STRINGS.UI.BUILTINCOMMANDS.ROLLBACK.DESC}
    }, {
        beta = false,
        pos = 'all',
        ...
        fn = {'confirm', 'c_rollback(6)', STRINGS.UI.BUILTINCOMMANDS.ROLLBACK.PRETTYNAME,
              STRINGS.UI.BUILTINCOMMANDS.ROLLBACK.DESC}
    }}
}, {
    ...
    list = {{
        ...
        fn = 'local p=%s local m=p and p.userid and p.player_classified and p.player_classified.MapExplorer local w,h = TheWorld.Map:GetSize() for x=-w*2,w*2,10 do for z=-h*2,h*2,10 do if TheWorld.Map:IsValidTileAtPoint(x,0,z) and m then m:RevealArea(x,0,z) end end end'
    }, {
        beta = false,
        pos = 'all',
        ...
        beta = false,
        pos = 'forest',
        name = STRINGS.TOO_MANY_ITEMS_UI.MAKE_WORMHOLE,
        tip = STRINGS.TOO_MANY_ITEMS_UI.MAKE_WORMHOLE_TIP,
        fn = 'local player = %s if player == nil then UserToPlayer("' .. _G.TOOMANYITEMS.DATA.ThePlayerUserId ..
            '").components.talker:Say("' .. STRINGS.TOO_MANY_ITEMS_UI.PLAYER_NOT_ON_SLAVE_TIP ..
    ...
    --     tip = "宣告当前世界种子",
    --     fn = function()
    --         ExecuteConsoleCommand('TheNet:Say(string.len(TheWorld.meta.seed) ~= 0 and TheWorld.meta.seed or TheWorld.meta.session_identifier)')
    --     end
    -- }
}
}, {
    ...
    list = {{
        ...
        pos = 'all',
        name = "出生门",
        tip = STRINGS.TOO_MANY_ITEMS_UI.DEBUG_GOTO_TELEPORTTIP .. STRINGS.NAMES.MULTIPLAYER_PORTAL .. '/' ..
            STRINGS.NAMES.MULTIPLAYER_PORTAL_MOONROCK,
        fn = gotoswitch('multiplayer_portal', 'multiplayer_portal_moonrock')
    }, {
        beta = false,
        pos = 'forest',
        name = "洞穴",
        tip = STRINGS.TOO_MANY_ITEMS_UI.DEBUG_GOTO_TELEPORTTIP .. STRINGS.NAMES.CAVE_ENTRANCE_OPEN .. '/' ..
        ...
        fn = gotoswitch('beequeenhive', 'beequeenhivegrown', 'beequeen')
    }, {
        beta = false,
        pos = 'forest',
        ...
        pos = 'forest',
        name = "泰拉瑞亚",
        tip = STRINGS.TOO_MANY_ITEMS_UI.DEBUG_GOTO_TELEPORTTIP .. "泰拉瑞亚",
        fn = gotoswitch('terrariumchest', 'terrarium')
    }, {
        beta = false,
        pos = 'forest',
        ...
        beta = false,
        pos = 'forest',
        name = STRINGS.NAMES.WORMHOLE,
        tip = STRINGS.TOO_MANY_ITEMS_UI.DEBUG_GOTO_TELEPORTTIP .. STRINGS.NAMES.WORMHOLE,
        fn = gotoswitch('wormhole')
    }, {
        beta = false,
        pos = 'forest',
        ...
        pos = 'forest',
        name = '水中木',
        tip = STRINGS.TOO_MANY_ITEMS_UI.DEBUG_GOTO_TELEPORTTIP .. STRINGS.NAMES.WATERTREE_PILLAR,
        fn = gotoswitch('watertree_pillar', 'watertree_root')
    }, {
        beta = false,
        pos = 'forest',
        name = '盐堆',
        tip = STRINGS.TOO_MANY_ITEMS_UI.DEBUG_GOTO_TELEPORTTIP .. STRINGS.NAMES.SALTSTACK,
        fn = gotoswitch('saltstack')
    }, {
        beta = false,
        pos = 'forest',
        name = '海草',
        tip = STRINGS.TOO_MANY_ITEMS_UI.DEBUG_GOTO_TELEPORTTIP .. STRINGS.NAMES.WATERPLANT,
        fn = gotoswitch('waterplant', 'waterplant_baby')
    }, {
        beta = false,
        pos = 'forest',
        name = '鱼群/邪天翁',
        tip = STRINGS.TOO_MANY_ITEMS_UI.DEBUG_GOTO_TELEPORTTIP .. '鱼群/邪天翁',
        fn = gotoswitch('malbatross', 'oceanfish_shoalspawner')
    },{
        beta = false,
        pos = 'forest',
        name = '猴女王',
        tip = STRINGS.TOO_MANY_ITEMS_UI.DEBUG_GOTO_TELEPORTTIP .. '猴女王',
        fn = gotoswitch('monkeyqueen')
    }, {
        beta = false,
        pos = 'cave',
        ...
        beta = false,
        pos = 'cave',
        name = STRINGS.NAMES.ARCHIVE_LOCKBOX_DISPENCER,
        tip = STRINGS.TOO_MANY_ITEMS_UI.DEBUG_GOTO_TELEPORTTIP .. STRINGS.NAMES.ARCHIVE_LOCKBOX_DISPENCER,
        fn = gotoswitch('archive_lockbox_dispencer', 'archive_lockbox')
    }, {
        beta = false,
        pos = 'cave',
        name = STRINGS.NAMES.ARCHIVE_SWITCH,
        tip = STRINGS.TOO_MANY_ITEMS_UI.DEBUG_GOTO_TELEPORTTIP .. STRINGS.NAMES.ARCHIVE_SWITCH,
        fn = gotoswitch('archive_switch')
    },{
        beta = false,
        pos = 'cave',
        name = "远古档案馆",
        tip = STRINGS.TOO_MANY_ITEMS_UI.DEBUG_GOTO_TELEPORTTIP .. "远古档案馆",
        fn = gotoswitch('archive_orchestrina_base')
    }, {
        beta = false,
        pos = 'cave',
        ...
        beta = false,
        pos = 'cave',
        name = STRINGS.NAMES.SLURTLEHOLE,
        tip = STRINGS.TOO_MANY_ITEMS_UI.DEBUG_GOTO_TELEPORTTIP .. STRINGS.NAMES.SLURTLEHOLE,
        fn = gotoswitch('slurtlehole')
    }, {
        beta = false,
        pos = 'cave',
        name = STRINGS.NAMES.SPIDERHOLE,
        tip = STRINGS.TOO_MANY_ITEMS_UI.DEBUG_GOTO_TELEPORTTIP .. STRINGS.NAMES.SPIDERHOLE,
        fn = gotoswitch('spiderhole')
    }, {
        beta = false,
        pos = 'cave',
        ...
        beta = false,
        pos = 'cave',
        name = STRINGS.NAMES.MUSHGNOME,
        tip = STRINGS.TOO_MANY_ITEMS_UI.DEBUG_GOTO_TELEPORTTIP .. STRINGS.NAMES.MUSHGNOME,
        fn = gotoswitch('mushgnome')
    }, {
        beta = false,
        pos = 'all',
