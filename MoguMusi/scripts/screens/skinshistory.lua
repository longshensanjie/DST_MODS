...
local SkinsHistory = Class(Screen, function(self,options)
	...
	end, str.clear, {200, 60}))
	self.clear:SetPosition(290, -240)
...
function SkinsHistory:BuildSkinsMenu()
	local function scrollWidgetCtorFn(context, index)
		...
		w.type:SetPosition(60, 40)

		w.date = w.bg:AddChild(Text(NEWFONT_OUTLINE, 28))
		w.date:SetPosition(45, -24)

		w.inst:DoTaskInTime(0, function()
			if ThePlayer and ThePlayer.HUD._StatusAnnouncer then
				w:SetClickable(true)
				w.OnMouseButton = function(_, button, down)
					if button == 1000 and down and TheInput:IsControlPressed(CONTROL_FORCE_INSPECT) and ThePlayer.HUD._StatusAnnouncer then
						local tp = w.type and w.type:GetString() or nil
						local dt = w.date:GetString() or nil
						local name = w.name:GetString() or nil

						if type(dt) == "string" then
							dt = string.gsub(dt, "%s", "  ") 
						end
						
						if tp and dt and name and type(tp) == "string" then
							if tp ~= "" then
								ThePlayer.HUD._StatusAnnouncer:Announce(string.format("我在『%s』通过『%s』获得了『%s』的皮肤。", dt, tp, name))
							else
								ThePlayer.HUD._StatusAnnouncer:Announce(string.format("我在『%s』获得了『%s』的皮肤。", dt, name))
							end
						end
					end
				end
	...
	local function scrollWidgetApplyFn(context, widget, data, index)
		...
		-- 皮肤类型
		if data.type then
			widget.type:SetString(data.type)
		else
			widget.type:SetString("")
		end
