...
local GeometricOptionsScreen = Class(Screen, function(self, modname, colorname_vectors, outlined_anims)
	...
	self.bg = self.proot:AddChild(TEMPLATES.RectangleWindow(619, 359, "几何布局 设置", close_button))
    ...
    self.subtitle_geometry:SetString("几何")
    ...
    self.subtitle_color:SetString("色彩")
    ...
    self.subtitle_color_good:SetString("允许")
    ...
    self.subtitle_color_bad:SetString("锁定")
    ...
    self.subtitle_gridsize1:SetString("建筑/作物")
    ...
    self.subtitle_gridsize2:SetString("墙体")
	...
	local placer_color_options = {
		{description = "绿色", data = "green", hover = "游戏中默认显示的绿色"},
		{description = "蓝色", data = "blue", hover = "蓝色, 希望可以帮助到红绿色盲"},
		{description = "红色", data = "red", hover = "游戏中默认显示的红色"},
		{description = "白色", data = "white", hover = "更加明亮的白色, 看起来会清晰许多"},
		{description = "黑色", data = "black", hover = "黑色，和其他亮色形成鲜明对比"},
	}
	local color_options = {}
	...
	local color_type_descriptions = {"作物/墙体", "", "地皮:", "", "建筑:", "", "描边:"}
	local color_type_hovers = {
		_GOOD = "你可以在这里放置建筑",
		_BAD = "位置被锁定或者遮挡, 无法放置建筑",
		[""] = "放置锚点的颜色, ",
		TILE = "放置地皮的颜色,",
		PLACER = "预览的物体颜色,",
		NEARTILE = "为锚点描边, 作为轮廓的颜色",
	}
	...
	self.color_buttons = {
		redgreen = { text = "红/绿", hover = "正常游戏中标准的红色和绿色", goodcolor = "green", badcolor = "red"},
		redblue = { text = "红/蓝", hover = "用蓝色替代绿色\n希望能帮助到红绿色盲", goodcolor = "blue", badcolor = "red"},
		blackwhite = { text = "黑/白", hover = "黑色表示被遮挡,白色表示可放置\n这样更显眼", goodcolor = "white", badcolor = "black"},
		blackwhiteoutline = { text = "黑白轮廓", hover = "黑色和而且带有轮廓\n非常显眼", goodcolor = "whiteoutline", badcolor = "blackoutline"},
		custom = { text = "自定义", hover = "自定义想要的色彩方案\n来搭配喜欢的布局颜色或者隐藏布局"},
		preset = { text = "预制", hover = "切换回预设选择模式\n让你快速选择系统配色方案"},
	}
    local button_y = 30
	...
	local geometries = {
		{name="Square", hover="与世界的 X-Z 坐标系对齐\n墙壁和地皮始终使用这样的正方形"},
		{name="Diamond", hover="正方形旋转了 45\176\n看起来还是正方形"},
		{name="X Hexagon", hover="平行于 X 轴的正六边形"},
		{name="Flat Hexagon", hover="和默认网格垂直的正六边形"},
		{name="Z Hexagon", hover="平行于 Z 轴的正六边形"},
		{name="Pointy Hexagon", hover="也是和默认网格垂直的正六边形"},
	}
	...
	local toggle_strings = {[true] = "关闭几何布局, 按住CTRL时才打开",
							[false]= "开启几何布局, 按住CTRL时才关闭"}
	...
	local toggle_buttons = {
		{name="grid", hover="是否有网格显示"},
		{name="placer", hover="是否显示放置预览\n(预览你种植的植物，放置的建筑等)"},
		{name="cursor", hover="是否在鼠标上显示携带的物品,\n只显示数量, 或者啥也不显示", toggle=2, atlases={"", "_num", ""}},
		{name="smart_spacing", hover="该功能被蘑菇慕斯作者移除, 无法打开！"},
		{name="till_grid", hover="是否在耕地上显示网格"},
	}
	...
	AddHoverText(self.refresh, {offset_x = -4, offset_y = 50}, "刷新网格的速度有多快。\n调高它会使其响应更快，但可能会导致卡顿。")
